...
在 loss 中加上参数**绝对值之和**：`Loss = 原始Loss + λΣ|wi|`

特点：会让一些参数变成 0 → 自动丢弃无用特征 → 起到**特征选择**的效果。

### L2 正则化（Ridge / Weight Decay）

在 loss 中加上参数**平方和**：`Loss = 原始Loss + λΣwi²`
...
- **精确率** = `TP / (TP + FP)` → 模型说"是"的里面，有多少真的是？（关注**误报**）
- **召回率** = `TP / (TP + FN)` → 实际为"是"的里面，模型抓到了多少？（关注**漏报**）
...
特点：如果 Precision 或 Recall 任一个很低，F1 也会很低。例子：Precision=100%, Recall=1% → 普通平均=50.5%（看似还行），F1≈2%（揭示真实水平）。
...
以真实值 `[100, 200, 300]`、预测值 `[110, 190, 280]`、误差 `[10, -10, -20]` 为例：**MAE** = (10+10+20)/3 ≈ **13.3 万**，**MSE** = (100+100+400)/3 = **200**，**RMSE** = √200 ≈ **14.1 万**。
